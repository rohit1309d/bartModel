...
class BartBPEABSAPipe(Pipe):
    ...
    def process(self, data_bundle: DataBundle) -> DataBundle:
        ...
        def prepare_target(ins):
            ...
            aspects = [a for a in ins['aspects']]
            #if self.opinion_first:
            #     aspects = sorted(aspects, key=cmp_to_key(cmp_opinion))
            # else:
            aspects_ = sorted(aspects, key=cmp_to_key(cmp_aspect))

            for aspects in aspects_:  # 预测bpe的start
